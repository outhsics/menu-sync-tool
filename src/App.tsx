...
function MenuTreePreview({ menus, level = 0 }: { menus: api.Menu[], level?: number }) {
  if (!menus || menus.length === 0) return <div className="text-xs text-muted-foreground p-2">无菜单数据</div>;
  
  return (
    <div className="text-sm font-mono">
      {menus.map(menu => (
        <div key={menu.id}>
          <div className="flex items-center gap-2 py-1 hover:bg-muted/50 rounded" style={{ paddingLeft: `${level * 16}px` }}>
             <span className="text-muted-foreground">{level > 0 ? '└─' : ''}</span>
             <span>{menu.name}</span>
             <Badge variant="outline" className="text-[10px] h-5 px-1">{menu.permission || '无权限标识'}</Badge>
          </div>
          {menu.children && <MenuTreePreview menus={menu.children} level={level + 1} />}
        </div>
      ))}
    </div>
  );
...
function App() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const _unusedTargetMenus = targetMenus; // Keep for future use or debugging
